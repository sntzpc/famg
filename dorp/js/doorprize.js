...
function renderStageWinners() {
  ...
  async function drawAnimatedAuto() {
    ...
    try {
      ...
      // Mode cepat untuk undian banyak (biar terasa instan)
      const maxTotalMs = (total > 10) ? 6000 : 12000;
      const revealMs = Math.max(120, Math.floor(maxTotalMs / Math.max(1,total)));
      const settleMs = Math.max(80, Math.floor(((total > 10) ? 900 : 2200) / Math.max(1,total)));
      const rollIntervalMs = Math.max(35, Math.floor(((total > 10) ? 120 : 180) / Math.max(1,total)));
  ...
  await Promise.all([refreshHistory(), loadPrizes()]);
