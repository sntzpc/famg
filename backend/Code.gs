...
function route_(action, p, token){
  switch(action){
    ...
    // ---- Public live location (tanpa token) ----
    case 'public.pushLiveLocation': return public_pushLiveLocation_(p);

    // ---- Admin live map ----
    case 'admin.liveLocations': return admin_only_('ADMIN', token, admin_liveLocations_);
...
function configDefault_(){
  ...
  return {
    ...
    app: {
      // ===== BRANDING (opsional) =====
      // Field ini dipakai frontend untuk menampilkan nama aplikasi/acara
      brand: {
...
function setup(){
  ...
  ensureSheet_(SH.config, ['key','value_json','updated_at','updated_by']);
  ensureSheet_(SH.live, ['nik','name','lat','lng','accuracy','speed','heading','ts_client','updated_at']);
  ensureSheet_(SH.liveHist, ['id','nik','name','lat','lng','accuracy','speed','heading','ts_client','created_at']);
...
// ==========================
// ✅ LIVE LOCATION (Public -> Store latest, Admin -> Read + in/out radius)
// ==========================
function haversineM_(lat1,lng1,lat2,lng2){
  const R = 6371e3;
  const toRad = (x)=> (Number(x)||0) * Math.PI/180;
...
// PUBLIC: peserta kirim lokasi
function public_pushLiveLocation_(p){
  const nik = String(p?.nik||'').trim();
  const loc = p?.loc || null;
  if(!nik) throw new Error('nik required');
  if(!loc || typeof loc !== 'object') throw new Error('loc required');

  const lat = Number(loc.lat);
  const lng = Number(loc.lng);
  if(!isFinite(lat) || !isFinite(lng)) throw new Error('invalid lat/lng');

  // anti-spam sederhana (per NIK minimal 3 detik)
  const cache = CacheService.getScriptCache();
  const k = 'LIVE_LOC_' + nik;
  const last = cache.get(k);
  if(last){
    // masih ok: tidak error, cukup return
    return { ok:true, skipped:true };
  }
  cache.put(k, '1', 3);

  // ambil nama peserta (kalau ada)
  const part = findParticipantByNik_(nik);
  ...
  const row = {
    ...
    lat,
    lng,
    accuracy: (loc.accuracy!=null)? Number(loc.accuracy) : '',
    speed: (loc.speed!=null)? Number(loc.speed) : '',
    heading: (loc.heading!=null)? Number(loc.heading) : '',
    ts_client: String(loc.ts||''),
    updated_at: nowIso_()
  };

  // upsert latest
  upsertByKey_(SH.live, 'nik', row);

  // history (opsional)
  const hid = Utilities.getUuid();
  const hist = Object.assign({ id: hid, created_at: nowIso_() }, row);
  sh_(SH.liveHist).appendRow([
    hist.id, hist.nik, hist.name, hist.lat, hist.lng,
    hist.accuracy, hist.speed, hist.heading, hist.ts_client, hist.created_at
  ]);

  return { ok:true };
}

// ADMIN: ambil semua live location + hitung dalam/luar radius
function admin_liveLocations_(p, u){
  ...
  const rows = liveRows.map(r=>{
    ...
    const dist = (isFinite(lat) && isFinite(lng))
      ? haversineM_(lat,lng,centerLat,centerLng)
      : null;

    const inRadius = (dist!=null && isFinite(radiusM) && radiusM>0) ? (dist <= radiusM) : false;
    ...
    return {
      ...
      distance_m: dist!=null ? dist : null,
      in_radius: inRadius
    };
  });

  // urutkan: yang update terbaru dulu
  rows.sort((a,b)=>{
...
function fmtAgo_(agoMs){
  const s = Math.max(0, Math.floor(agoMs/1000));
  if(s < 60) return s+'s';
